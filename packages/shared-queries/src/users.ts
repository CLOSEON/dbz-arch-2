...
export async function getApprovedVendors(forceFresh = false): Promise<Vendor[]> {
  const now = Date.now();
  if (!forceFresh && _vendorsCache && now - _vendorsCache.ts < VENDORS_CACHE_TTL) {
...
//
// The per-app copies of this module disagreed on cache TTLs, and neither value
// was wrong: web-main / admin-panel / vendor-panel used 5 minutes ("prevents
// repetitive reads on frequent updates"), while rider-panel deliberately used
// 20 seconds so riders see fresh vendor/profile data mid-delivery. Picking one
// would have either 15x'd Firestore reads for three apps or made rider data
// up to 5 minutes stale.
//
// So the TTLs are configurable, defaulting to the conservative 5 minutes.
// rider-panel opts into the shorter window at startup via
// configureUserCacheTTLs(). See IMPLEMENTATION_PLAN.md Phase 1.
const DEFAULT_PROFILE_CACHE_TTL = 300_000;
const DEFAULT_VENDORS_CACHE_TTL = 300_000;

let PROFILE_CACHE_TTL = DEFAULT_PROFILE_CACHE_TTL;
let VENDORS_CACHE_TTL = DEFAULT_VENDORS_CACHE_TTL;

/**
 * Override the user/vendor cache windows for this app.
 * Call once during app startup, before any query runs.
 */
export function configureUserCacheTTLs(opts: { profileMs?: number; vendorsMs?: number }) {
  if (typeof opts.profileMs === 'number') PROFILE_CACHE_TTL = opts.profileMs;
  if (typeof opts.vendorsMs === 'number') VENDORS_CACHE_TTL = opts.vendorsMs;
}
...
// Previously existed only in vendor-panel's copy of this module (and only
// vendor-panel calls it). Kept here so the query layer has one definition.
