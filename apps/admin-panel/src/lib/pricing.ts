 ...
 *
 * Backwards-compatible with explicit margin percentage (e.g. 40% or 50%).
 */
export function calculateVendorPayout(
  ...
  // If passed an explicit standard meal payout amount (e.g. ₹60, ₹65, ₹70):
  if (vendorMarginOrPayout >= 50) {
    const payout = rawKitchenCost * (vendorMarginOrPayout / safeBaseRaw);
    return Math.round(payout * 100) / 100;
  }

  // Otherwise interpreted as vendor margin percent (e.g. 40%):
