...
function DashboardScreen() {
  return (
    <div className="w-full h-full bg-[#FEFCE8] flex flex-col text-slate-900 overflow-hidden select-none">
      {/* Red Header */}
      <div className="bg-[#FF3B30] px-4 pt-4 pb-8 flex flex-col gap-1">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-2">
            <div className="bg-[#FFCC00] rounded-full px-2.5 py-1">
              <span className="text-[#1a1a1a] text-[9px] font-black">Dabzzo.in</span>
            </div>
            <Bell className="w-4 h-4 text-white" />
          </div>
        </div>
        <p className="text-white/80 text-[9px] font-semibold uppercase tracking-widest">Good afternoon, test</p>
      ...
      <div className="mx-3 -mt-5 bg-slate-900 rounded-2xl p-3 shadow-xl z-10">
        <div className="flex items-center justify-between mb-1.5">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-green-400 text-[8px] font-black uppercase tracking-widest">Live Order Status</span>
          </div>
        ...
        <div className="w-full bg-[#FF3B30] rounded-lg py-1.5 text-center">
          <span className="text-white text-[9px] font-black tracking-wide">TRACK LIVE DELIVERY</span>
        </div>
      </div>

      {/* Search */}
      <div className="mx-3 mt-3 flex items-center gap-2 bg-white rounded-xl px-3 py-2 shadow-sm border border-slate-100">
      ...
      <div className="grid grid-cols-3 gap-2 px-3 mt-3">
        {[
          ...
          <div key={label} className="bg-white rounded-xl p-2 shadow-sm border border-slate-100 text-center">
            <div className="w-6 h-6 bg-[#FF3B30]/10 rounded-lg flex items-center justify-center mx-auto mb-1">
      ...
      <div className="px-3 mt-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1 text-[8px] text-slate-400">
          <span className="text-[#FF3B30] font-bold">↺ 1 FOUND</span>
        </div>
      </div>
      <div className="mx-3 mt-2 bg-white rounded-2xl p-3 shadow-sm border border-slate-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="flex items-center gap-1 mb-0.5">
              <span className="text-[8px] font-black text-green-600 bg-green-50 border border-green-200 rounded-full px-1.5 py-0.5">✓ VERIFIED</span>
            </div>
      ...
      <div className="mt-auto border-t border-slate-100 bg-white flex items-center justify-around px-4 py-2">
        {[{ icon: Home, label: 'HOME', active: true }, { icon: ShoppingBag, label: 'ORDERS', active: false }, { icon: User, label: 'PROFILE', active: false }].map(({ icon: Icon, label, active }) => (
          <div key={label} className="flex flex-col items-center gap-0.5">
            <Icon className={`w-4 h-4 ${active ? 'text-[#FF3B30]' : 'text-slate-400'}`} />
            <span className={`text-[7px] font-black ${active ? 'text-[#FF3B30]' : 'text-slate-400'}`}>{label}</span>
            {active && <div className="w-1 h-1 rounded-full bg-[#FF3B30]" />}
          </div>
        ))}
      </div>
    </div>
...
function OrdersScreen() {
  return (
    <div className="w-full h-full bg-[#FEFCE8] flex flex-col text-slate-900 overflow-hidden select-none">
      <div className="bg-[#FF3B30] px-4 pt-5 pb-5">
        <p className="text-white/80 text-[9px] font-semibold uppercase tracking-widest mb-1">Your Orders</p>
      ...
      <div className="flex-1 px-3 pt-4 space-y-3 overflow-hidden">
        {[
          { status: 'Delivered', meal: 'Dal Rice + Sabzi', date: 'Today, Lunch', color: 'bg-green-50 border-green-200', badge: 'bg-green-500' },
          { status: 'Scheduled', meal: 'Roti + Paneer', date: 'Tomorrow, Lunch', color: 'bg-blue-50 border-blue-200', badge: 'bg-blue-500' },
...
export function AppPreview() {
  return (
    <section className="py-24 bg-slate-900 text-white overflow-hidden relative">
      {/* Subtle texture */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_rgba(255,59,48,0.15),transparent_60%)] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-16">
          <motion.p
            ...
            viewport={{ once: true }}
            className="text-brand font-bold tracking-widest uppercase text-sm mb-3"
          >
            App Preview
          </motion.p>
          <motion.h2
        ...
        <div className="flex items-end justify-center gap-4 sm:gap-8 min-h-[480px] pb-8">
          ...
          <PhoneMockup rotate={6} delay={0.3} zIndex={10}>
            <div className="w-full h-full bg-[#FEFCE8] flex flex-col overflow-hidden select-none">
              <div className="bg-[#FF3B30] px-4 pt-5 pb-5">
                <p className="text-white/80 text-[9px] font-semibold uppercase tracking-widest mb-1">Your Profile</p>
              ...
              <div className="flex-1 px-3 pt-4 space-y-2.5">
                {[
                  ...
                  <div key={item.label} className="bg-white rounded-2xl p-3 flex items-center gap-3 shadow-sm border border-slate-100">
                ...
                <div className="bg-[#FF3B30] rounded-2xl p-3 text-center">
