...
export default function UserDashboard() {
  ...
  useEffect(() => {
    ...
    const qDeliveries = query(
      ...
      where('status', 'in', ['pending', 'preparing', 'ready', 'picked_up', 'out_for_delivery'])
    );
    const unsubDeliveries = onSnapshot(qDeliveries, (snap) => {
      if (!snap.empty) {
        // Take the most relevant active delivery order
  ...
  return (
    <div style={{ background: '#FEFCE8', minHeight: '100dvh' }}>
      ...
      <div className="px-5 pb-6 sm:px-6" style={{ background: '#FEFCE8' }}>
        ...
        {/* ── Blended Benefits Grid ── */}
        <div className="grid grid-cols-3 gap-3 mb-6 animate-fade-in">
          <div className="bg-white rounded-2xl p-3 border border-slate-100 shadow-sm flex flex-col items-center text-center">
            <div className="w-8 h-8 rounded-xl bg-red-50 flex items-center justify-center text-brand mb-2">
              <ArrowLeftRight className="w-4 h-4" strokeWidth={2.4} />
            </div>
            <h4 className="text-[11px] font-bold text-slate-800">Menu Swaps</h4>
            <p className="text-[9px] text-slate-400 mt-0.5 leading-tight">Switch kitchens anytime</p>
          </div>
          <div className="bg-white rounded-2xl p-3 border border-slate-100 shadow-sm flex flex-col items-center text-center">
            <div className="w-8 h-8 rounded-xl bg-amber-50 flex items-center justify-center text-amber-500 mb-2">
              <CalendarCheck2 className="w-4 h-4" strokeWidth={2.4} />
            </div>
            <h4 className="text-[11px] font-bold text-slate-800">Easy Pause</h4>
            <p className="text-[9px] text-slate-400 mt-0.5 leading-tight">Save credits when away</p>
          </div>
          <div className="bg-white rounded-2xl p-3 border border-slate-100 shadow-sm flex flex-col items-center text-center">
            <div className="w-8 h-8 rounded-xl bg-emerald-50 flex items-center justify-center text-emerald-500 mb-2">
              <Zap className="w-4 h-4" strokeWidth={2.4} />
            </div>
            <h4 className="text-[11px] font-bold text-slate-800">Live Track</h4>
            <p className="text-[9px] text-slate-400 mt-0.5 leading-tight">Real-time GPS delivery</p>
          </div>
        </div>
