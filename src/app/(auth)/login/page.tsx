...
import { Sparkles, ArrowRight, ShieldCheck, Utensils } from 'lucide-react';
  ...
  const handleAuthSuccess = useCallback(async (firebaseUser: any) => {
    ...
    try {
      ...
      if (isAdmin) {
        ...
        addToast(`Welcome back, ${finalProfile.name || 'there'}! ✨`, 'success');
      }
    ...
    >
          ...
          {step === 'onboarding' && (
            <form onSubmit={handleOnboarding} className="w-full space-y-4 animate-fade-in">
              ...
              {!isExistingUserMissingName && (
                <div className="pt-2">
                  <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest block mb-2.5 ml-1">
                    Account Type
                  </label>
                  <div className="flex flex-col gap-2.5">
                    {([
                      { role: 'user' as UserRole, title: 'Customer', desc: 'Order and manage daily subscriptions' },
                      { role: 'vendor' as UserRole, title: 'Kitchen Partner', desc: 'List and sell homemade tiffins' },
                      { role: 'delivery' as UserRole, title: 'Delivery Partner', desc: 'Deliver meal orders & earn' },
                    ]).map(({ role, title, desc }) => (
                      <button
                        key={role}
                        type="button"
                        onClick={() => setSelectedRole(role)}
                        className={`w-full flex items-center justify-between p-3.5 rounded-2xl border-2 transition-all active:scale-[0.99] text-left ${
                          selectedRole === role
                      ...
                      >
                        <div>
                          <p className={`font-black text-sm ${selectedRole === role ? 'text-amber-950' : 'text-slate-800'}`}>
                            {title}
                          </p>
                          <p className="text-xs font-medium text-slate-500">{desc}</p>
                        </div>
                        <div className={`w-4.5 h-4.5 rounded-full border-2 flex items-center justify-center ${selectedRole === role ? 'border-amber-600 bg-white' : 'border-slate-300'}`}>
                          {selectedRole === role && <div className="w-2.5 h-2.5 bg-amber-600 rounded-full" />}
                        </div>
                      </button>
                    ))}
                  </div>
      ...
      <div className="pt-6 relative z-10">
        <p className="text-xs text-slate-500 font-medium text-center">
          Dabzzo • Fresh Home Cooked Tiffins • <span className="font-bold underline decoration-slate-300 underline-offset-2">Terms</span> & <span className="font-bold underline decoration-slate-300 underline-offset-2">Privacy</span>
        </p>
      </div>
    </div>
  );
